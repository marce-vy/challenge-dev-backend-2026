...
class RetryFactoryTest {
  ...
  void createsRetryWithThreeTotalAttemptsAndBackoff() {
    Retry retry =
        RetryFactory.withExponentialBackoff("test-percentage-provider", 3, Duration.ofMillis(50), 2.0);
  ...
  void retriesTransientExceptionsOnly() {
    ...
    assertThat(retry.getRetryConfig().getExceptionPredicate().test(new java.io.IOException("timeout")))
        .isTrue();
    assertThat(
