...
class GlobalExceptionHandlerTest {
  ...
  @Test
  void handleServerWebInputReturns400ForBodyErrors() {
    MethodParameter bodyParam = mock(MethodParameter.class);
    when(bodyParam.hasParameterAnnotation(RequestBody.class)).thenReturn(true);
    ServerWebInputException ex = new ServerWebInputException("read error", bodyParam, null);

    ResponseEntity<ErrorResponse> response = handler.handleServerWebInput(ex);
  ...
  @Test
  void handleServerWebInputReturns400ForQueryParamErrors() {
    MethodParameter queryParam = mock(MethodParameter.class);
    when(queryParam.hasParameterAnnotation(RequestBody.class)).thenReturn(false);
    ServerWebInputException ex = new ServerWebInputException("type mismatch", queryParam, null);
